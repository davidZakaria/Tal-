  ...
  const handlePayReservation = async (reservationId: string) => {
    ...
    try {
      ...
      if (!payRes.ok) {
        ...
        if (data?.code === "PAYMOB_DISABLED" || payRes.status === 503) {
          setPaymobEnabled(false);
          alert(data?.message || "Online card payment is under development. Please contact our team.");
          return;
        }
      ...
      if (data.iframe_url) {
        ...
        alert(data.message || "Online card payment is under development.");
      }
    } catch {
                 ...
                 <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i*0.1 }} key={res._id} className="bg-white rounded-[2.5rem] shadow-xl shadow-sapphire/5 border border-sapphire/5 overflow-hidden group">
                   ...
                   <div className="p-8">
                      ...
                      <div className="mt-6 flex justify-between items-end gap-4 flex-wrap">
                         ...
                         {res.status === "ApprovedAwaitingPayment" && paymobEnabled && (
                           <button
                             type="button"
                      ...
                      {res.status === "ApprovedAwaitingPayment" && !paymobEnabled && (
                        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sapphire">
                          <div className="flex items-center gap-2 mb-2">
                            <CreditCard className="w-4 h-4 text-amber-600" />
                            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-700">
                              Online payment under development
                            </p>
                          </div>
                          <p className="text-xs leading-relaxed text-sapphire/80">
                            Please contact our team to settle this reservation via bank transfer or at reception. Your
                            booking will move to <span className="font-semibold">Confirmed</span> once payment is
                            recorded by our staff.
                          </p>
                        </div>
                      )}
                   </div>
