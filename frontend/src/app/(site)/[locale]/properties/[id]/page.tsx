...
export default function PropertyDetails() {
  ...
  const [arrivalDate, setArrivalDate] = useState(() => {
    ...
    return searchParams.get("checkIn") || getBookingIntent()?.checkIn || "";
  });
  const [departureDate, setDepartureDate] = useState(() => {
    if (typeof window === "undefined") return "";
    return searchParams.get("checkOut") || getBookingIntent()?.checkOut || "";
  });
  const [guests, setGuests] = useState(() => {
    ...
    return getBookingIntent()?.guests ?? 2;
  });
  ...
  const suitesHref = `/${locale}#suites`;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-brand-charcoal">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-brand-white/50">{t("loadingSuite")}</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !property) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-8 bg-brand-charcoal px-6 text-center">
        <p className="font-serif text-2xl text-brand-white/90 md:text-3xl">{t("notFound")}</p>
        <Link
          href={suitesHref}
  ...
  return (
    <div className="min-h-screen min-w-0 bg-brand-charcoal text-brand-white selection:bg-brand-gold/30">
      <header className="sticky top-0 z-50 border-b border-black/10 bg-brand-teal shadow-[0_8px_32px_rgba(0,0,0,0.18)]">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-4 sm:gap-6 sm:px-6 md:px-10 md:py-5">
          ...
          >
            ...
            >
              <ArrowLeft className="h-4 w-4 shrink-0 opacity-90 rtl:-scale-x-100" aria-hidden />
              {t("backAria")}
            </button>
          ...
          <div className="relative aspect-video w-full max-h-[min(68vh,760px)] sm:aspect-[21/9]">
            {property.images && property.images.length > 0 ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-brand-teal/40">
                <span className="text-sm font-bold uppercase tracking-[0.3em] text-brand-white/40">{t("noImagery")}</span>
              </div>
            )}
            ...
            <div className="absolute bottom-0 start-0 end-0 z-10 p-6 sm:p-10 md:p-12">
              <motion.div
      ...
      <div className="mx-auto max-w-7xl px-4 pb-28 pt-10 sm:px-6 md:px-10 md:pt-14">
        ...
        >
          <Link href={homeHref} className="transition-colors hover:text-brand-gold">
            {t("breadcrumbHome")}
          </Link>
          <span className="text-brand-gold/40">/</span>
          <Link href={suitesHref} className="transition-colors hover:text-brand-gold">
            {t("breadcrumbSanctuaries")}
          </Link>
          <span className="text-brand-gold/40">/</span>
          ...
          <div className="space-y-14 lg:col-span-7">
            ...
            >
              <div className="mb-6 flex items-center gap-2">
                ...
                <h2 className="text-[11px] font-bold uppercase tracking-[0.35em] text-brand-gold/90">{t("experienceLabel")}</h2>
              </div>
              <p className="font-serif text-xl font-light leading-relaxed text-brand-white/90 sm:text-2xl md:text-[1.65rem] md:leading-[1.55]">
                {property.description || t("descriptionDefault")}
              </p>
            </motion.section>
            ...
            >
              <h2 className="mb-8 text-[11px] font-bold uppercase tracking-[0.35em] text-brand-gold/90">{t("amenitiesLabel")}</h2>
                    ...
                    .map((dateStr, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {dateObj.toLocaleDateString(dateLocale, { month: "short", day: "numeric", year: "numeric" })}
                        </span>
                      );
                    })}
                </div>
          ...
          <div className="lg:col-span-5">
            ...
            >
              <div className="mb-8 border-b border-brand-gold/20 pb-8">
                ...
                <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.28em] text-brand-white/45">{t("pricePerNight")}</p>
              </div>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-brand-white/15 bg-brand-charcoal/50 p-3 transition-colors focus-within:border-brand-gold/40 focus-within:bg-brand-charcoal/70 sm:p-4">
                    <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-brand-gold/75">{t("arrival")}</label>
                    <input
                      type="date"
                      value={arrivalDate}
                  ...
                  <div className="rounded-2xl border border-brand-white/15 bg-brand-charcoal/50 p-3 transition-colors focus-within:border-brand-gold/40 focus-within:bg-brand-charcoal/70 sm:p-4">
                    <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-brand-gold/75">{t("departure")}</label>
                    <input
                      type="date"
                      value={departureDate}
                ...
                <div className="rounded-2xl border border-brand-white/15 bg-brand-charcoal/50 p-3 transition-colors focus-within:border-brand-gold/40 focus-within:bg-brand-charcoal/70 sm:p-4">
                  <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-brand-gold/75">{t("guests")}</label>
                  <select
                    value={guests}
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!reqRes.ok) {
                        ...
                        setPaymentError(data.message || data.error || t("requestFailed"));
                        setIsProcessing(false);
                        return;
                      }
                ...
                >
                  ...
                  {hasConflict ? t("datesConflict") : isProcessing ? t("submitting") : t("requestReservation")}
                </button>
