...
import { Search, Filter, MoreVertical, CreditCard, Clock, Calendar, MessageSquare, CheckCircle, Info } from "lucide-react";
import { apiUrl } from "@/lib/api";
import { adminFetch, AdminSessionInvalidError } from "@/lib/adminAuth";
import { fetchPaymentConfig } from "@/lib/paymentConfig";
...
export default function AdminReservations() {
  ...
  const [paymobEnabled, setPaymobEnabled] = useState<boolean>(false);

  const load = async () => {
    try {
      const res = await adminFetch(apiUrl("/api/inventory/reservations"));
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    fetchPaymentConfig(controller.signal).then((cfg) => setPaymobEnabled(cfg.paymobCheckout));
    return () => controller.abort();
  }, []);

  const handleApprove = async (id: string) => {
    try {
      const resp = await adminFetch(apiUrl(`/api/inventory/reservations/${id}/approve`), {
        method: "PATCH",
      });
      if (resp.ok) load();
      else {
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      const resp = await adminFetch(apiUrl(`/api/inventory/reservations/${id}/reject`), {
        method: "PATCH",
      });
      if (resp.ok) load();
      else {
  ...
  const handleManualConfirm = async (id: string) => {
    if (
      !confirm(
        "Record payment received (bank transfer, cash, or at reception)? This will confirm the booking and send the guest their confirmation email."
      )
    )
      return;
    try {
      const resp = await adminFetch(apiUrl(`/api/inventory/reservations/${id}/confirm`), {
        method: "PATCH",
      });
      if (resp.ok) load();
      else {
  ...
  return (
    ...
    >
      {!paymobEnabled && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-4 flex items-start gap-4 shadow-sm">
          <div className="shrink-0 w-10 h-10 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center">
            <Info className="w-5 h-5" />
          </div>
          <div className="flex-1">
            <p className="text-[11px] uppercase tracking-[0.25em] font-bold text-amber-800 mb-1">
              Online card payments — Under development
            </p>
            <p className="text-sm text-sapphire/80 leading-relaxed">
              PayMob checkout is currently disabled. After approving a request, settle payment with the guest by bank
              transfer, cash, or at reception and press <span className="font-semibold">Record payment</span> on the
                      ...
                      <div className="flex flex-col gap-2 items-end">
                        ...
                        {res.status === "ApprovedAwaitingPayment" && (
                          <button
                            ...
                            title={
                              paymobEnabled
                                ? "Record an offline payment (bank transfer, cash, or at reception)"
                                : "Online card payment is under development — record payment manually to confirm"
                            }
                            className="px-4 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
                          >
                            Record payment
                          </button>
                        )}
