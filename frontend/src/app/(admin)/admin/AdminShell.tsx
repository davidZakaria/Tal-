...
import { LayoutDashboard, Home, ReceiptText, Sparkles, LogOut, LockKeyhole, Loader2 } from "lucide-react";
import { usePathname, useRouter } from "next/navigation";
import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { apiUrl, describeApiBase } from "@/lib/api";
import { ADMIN_TOKEN_KEY, getAdminToken } from "@/lib/adminAuth";
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const menu = [
    ...
    { name: "Presentation Leads", icon: Sparkles, path: "/admin/leads" },
  ];

  if (isChecking)
    return (
      <div className="min-h-screen bg-sand-light flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-sapphire" />
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-sapphire flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <motion.div
          ...
          className="bg-white/10 backdrop-blur-xl p-12 rounded-[3rem] border border-white/10 shadow-2xl w-full max-w-md relative z-10 text-center"
        >
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            />
            {loginError && (
              <p className="text-red-400 text-xs font-bold tracking-widest" role="alert">
                {loginError}
              </p>
            )}
            <Button
              ...
              className="w-full !bg-turquoise !text-sapphire hover:!bg-white shadow-[0_0_20px_rgba(48,197,210,0.3)]"
            >
  ...
  return (
    <div className="min-h-screen bg-sand-light flex">
      <aside className="w-72 bg-sapphire text-sand-light hidden md:flex flex-col shadow-2xl z-20">
        ...
        <div className="p-6">
          <button
            ...
            className="flex items-center gap-5 px-6 py-4 w-full text-sand-light/60 hover:text-terracotta hover:bg-terracotta/10 rounded-3xl transition-all"
          >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 md:px-12 py-10">{children}</div>
      </main>
    </div>
  );
}
